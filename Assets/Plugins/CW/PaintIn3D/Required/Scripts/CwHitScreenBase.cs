...
namespace PaintIn3D
{
    ...
    public abstract class CwHitScreenBase : CwHitPointers
    {
        ...
        [UnityEngine.Serialization.FormerlySerializedAs("draw")] [SerializeField]
        ...
        [UnityEngine.Serialization.FormerlySerializedAs("normal")] [SerializeField]
        ...
        {
            set { normalRelativeTo = value; }
            get { return normalRelativeTo; }
        }

        [UnityEngine.Serialization.FormerlySerializedAs("orientation")] [SerializeField]
        ...
        {
            set { storeStates = value; }
            get { return storeStates; }
        }
        ...
        {
            get { return rotateTo == RotationType.Normal && normalRelativeTo == RelativeType.DrawAngle; }
        }
        ...
        protected virtual void DoQuery(Vector2 screenPosition, ref Camera camera, ref Ray ray, ref CwHit hit3D,
        ...
        protected void PaintAt(CwPointConnector connector, CwHitCache hitCache, Vector2 screenPosition,
            Vector2 screenPositionOld, bool preview, float pressure, object owner)
        {
            ...
            {
                CalcHitData(hit3D.Position, hit3D.Normal, ray, camera, screenPositionOld, ref finalPosition,
                    ref finalRotation);
                ...
                if (emit == EmitType.PointsIn3D)
                {
                    if (connector != null)
                    {
                        connector.SubmitPoint(gameObject, preview, priority, pressure, finalPosition, finalRotation, owner, hit3D.Collider.name);
                    }
                    else
                    {
                        hitCache.InvokePoint(gameObject, preview, priority, pressure, finalPosition, finalRotation, hit3D.Collider.name);
                    }
            ...
            {
                CalcHitData(hit2D.point, new Vector3(0.0f, 0.0f, -1.0f), ray, camera, screenPositionOld,
                    ref finalPosition, ref finalRotation);

                if (emit == EmitType.PointsIn3D)
                {
                    if (connector != null)
                    {
                        connector.SubmitPoint(gameObject, preview, priority, pressure, finalPosition, finalRotation,
                            owner);
                    }
                    else
                    {
        ...
        private void CalcHitData(Vector3 hitPoint, Vector3 hitNormal, Ray ray, Camera camera, Vector2 screenPositionOld,
            ref Vector3 finalPosition, ref Quaternion finalRotation)
        {
                ...
                case RotationType.Normal:
                {
                    ...
                    switch (normalDirection)
                    {
                        ...
                        case DirectionType.RayDirection:
                            finalNormal = -ray.direction;
                            break;
                        case DirectionType.CameraDirection:
                            finalNormal = -camera.transform.forward;
                            break;
                    }
                    ...
                    switch (normalRelativeTo)
                    {
                        ...
                        case RelativeType.DrawAngle:
                        {
                            ...
                            {
                                finalUp = Vector3.Cross(rayOld.GetPoint(1.0f) - ray.origin,
                                    ray.GetPoint(1.0f) - ray.origin);
                            }
                            else
                            {
