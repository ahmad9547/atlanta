...
namespace PaintCore
{
	...
	public class CwHitCache
	{
		...
		public void InvokePoint(GameObject gameObject, bool preview, int priority, float pressure, Vector3 position, Quaternion rotation, string name = "")
		{
			if (cached == false)
			{
				Cache(gameObject);
			...
			for (var i = 0; i < hitPoints.Count; i++)
			{
				hitPoints[i].HandleHitPoint(preview, priority, pressure, seed, position, rotation, name);
			}
		}

		public void InvokeLine(GameObject gameObject, bool preview, int priority, float pressure, Vector3 position, Vector3 endPosition, Quaternion rotation, bool clip, string name = "")
		{
			if (cached == false)
			{
				Cache(gameObject);
			...
			for (var i = 0; i < hitLines.Count; i++)
			{
				hitLines[i].HandleHitLine(preview, priority, pressure, seed, position, endPosition, rotation, clip, name);
			}
		}
