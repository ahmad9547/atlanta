...
namespace PaintCore
{
	...
	public class CwPointConnector
	{
		...
		public void SubmitPoint(GameObject gameObject, bool preview, int priority, float pressure, Vector3 position, Quaternion rotation, object owner, string name = "")
		{
			if (owner != null)
			{
				...
				if (TryGetLink(owner, ref link) == true)
				{
					if (hitSpacing > 0.0f && preview == false)
					{
						...
						for (var i = 0; i < steps; i++)
						{
							...
							if (connectHits == true)
							{
								hitCache.InvokeLine(gameObject, preview, priority, pressure, link.Position, currentPosition, rotation, clipConnected, name);
							}
							else
							{
								hitCache.InvokePoint(gameObject, preview, priority, pressure, currentPosition, rotation, name);
							}
					...
					{
						hitCache.InvokeLine(gameObject, preview, priority, pressure, link.Position, position, rotation, clipConnected, name);
					}
					else
					{
						hitCache.InvokePoint(gameObject, preview, priority, pressure, position, rotation, name);
					}
				}
				else
				{
