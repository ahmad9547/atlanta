...
Shader "Custom/URPVegLeaves"
{
	Properties
	{
		...
		_Cutoff("Alpha Cutoff", Range( 0 , 1)) = 0.223
		[Enum(No,2,Yes,0)] _TwoSided ("Two Sided", Int) = 2
		_NormalMap("Normal Map", 2D) = "bump" {}
		_TranslucencyMap("Translucency Map", 2D) = "white" {}
		[Header(Visual Settings)]
		_SpecularColor("Specular Color", Color) = (0.05882353,0.0566609,0.0566609,0)
		_SpecularStrength("Specular Strength", Range( 0 , 1)) = 0.2
		_TransAmbient("Ambient", Range( 0, 1 ) ) = 0.2
		_TransScattering("Ambient Scattering", Range( 1, 50 ) ) = 2
		_TransStrength("Translucency Strength", Range( 0, 50 ) ) = 1
		_TranslucencySaturation("Translucency Saturation", Range( 0 , 2)) = 0.4
		_TransDirect( "Translucency Direction", Range( 0, 1 ) ) = 1
		_Distortion( "Distortion", Range( 0, 1 ) ) = 0.1
		_TransShadow( "Shadow", Range( 0, 1 ) ) = 0.5
		_ShadowStrength("Shadow Strength", Range( 0 , 1)) = 0
		_ShadowSaturation("Shadow Saturation", Range( 0 , 1)) = 0.75
		...
		NoiseTextureFloat1("Noise Texture", 2D) = "white" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			VertexOutput VertexFunction( VertexInput v  )
			{
				...
				#endif
				float3 vertexValue = staticSwitch176.xyz;
				#ifdef ASE_ABSOLUTE_VERTEX_POS
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float temp_output_2_0_g3 = _ShadowSaturation;
				...
				float4 lerpResult62 = lerp( ( dotResult58 * tex2DNode25 ) , tex2DNode25 , _ShadowStrength);
				...
				float3 lerpResult89 = lerp( float3( 0,0,0 ) , float3( 0,0,0 ) , desaturateVar90);
				float4 lerpResult91 = lerp( temp_output_74_0 , float4( lerpResult89 , 0.0 ) , float4( 0,0,0,0 ));
				
				float2 uv_TranslucencyMap = IN.ase_texcoord7.xy * _TranslucencyMap_ST.xy + _TranslucencyMap_ST.zw;
				float3 desaturateInitialColor39 = tex2D( _TranslucencyMap, uv_TranslucencyMap ).rgb;
				float desaturateDot39 = dot( desaturateInitialColor39, float3( 0.299, 0.587, 0.114 ));
				float3 desaturateVar39 = lerp( desaturateInitialColor39, desaturateDot39.xxx, _TranslucencySaturation );
				...
				float3 Specular = ( tex2DNode25 * _SpecularColor ).rgb;
				float Metallic = 0;
				float Smoothness = _SpecularStrength;
				float Occlusion = 1;
				...
				float3 Translucency = ( temp_output_78_0 * desaturateVar39 );
		...
		Pass
		{
			...
			VertexOutput VertexFunction( VertexInput v )
			{
				...
				float2 panner158 = ( 1.0 * _Time.y * ( temp_output_153_0 * WindSpeedFloat1 * 10.0 ).xy + (ase_worldPos).xy);
				float4 Noise166 = ( tex2Dlod( NoiseTextureFloat1, float4( ( ( panner158 * WindTurbulence ) / float2( 10,10 ) ), 0, 0.0) ) * _WindStrength * 0.5 );
				float4 transform174 = mul(GetWorldToObjectMatrix(),( float4( _WindDirection , 0.0 ) * ( ( v.ase_color * Noise166 ) + ( Noise166 * v.ase_color ) ) ));
				#ifdef _WIND
				float4 staticSwitch176 = transform174;
				#else
				float4 staticSwitch176 = float4( 0,0,0,0 );
				#endif
